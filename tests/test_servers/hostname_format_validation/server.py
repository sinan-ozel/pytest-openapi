...
def _is_valid_hostname(value):
    ...
    if len(value) > 253:
        return False, f"Hostname exceeds maximum length of 253 characters (got {len(value)})"
    labels = value.rstrip(".").split(".")
    if not labels:
        return False, "Hostname must contain at least one label"
    for label in labels:
        if not label:
            return False, "Hostname must not contain consecutive dots or a leading dot"
                            ...
                            "200": {
                                ...
                                "content": {
                                    "application/json": {
                                        ...
                                        "example": {"id": 1, "status": "configured"},
                                    }
                                },
                            },
                            "400": {
...
def configure_server():
    ...
    if not valid:
        return (
            jsonify({"error": "Invalid hostname", "field": "server_hostname", "detail": reason}),
            400,
        )
